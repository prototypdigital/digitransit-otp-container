...
gulp.task(
  "gtfs:fit",
  gulp.series("del:filter", "hslHack", function() {
    return (
      gulp
        .src([`${config.dataDir}/fit/gtfs/*`])
...
gulp.task("copyRouterConfig", function() {
  return gulp.src(["router-*/**"]).pipe(gulp.dest(config.dataDir));
});
...
gulp.task(
  "gtfs:filter",
  gulp.series("copyRouterConfig", function() {
    return (
      gulp
        .src([`${config.dataDir}/filter/gtfs/*`])
...
gulp.task(
  "gtfs:seed",
  gulp.series("gtfs:del", function() {
    return Seed(config.ALL_CONFIGS(), /\.zip/).pipe(
      gulp.dest(`${config.dataDir}/ready/gtfs`)
    );
  })
);
...
gulp.task(
  "osm:seed",
  gulp.series("osm:del", function() {
    return Seed(config.ALL_CONFIGS(), /.pbf/).pipe(
      gulp.dest(`${config.dataDir}/ready/osm`)
    );
  })
);
...
gulp.task(
  "dem:seed",
  gulp.series("dem:del", function() {
    return Seed(config.ALL_CONFIGS(), /.tif/).pipe(
      gulp.dest(`${config.dataDir}/ready/dem`)
    );
  })
);
...
gulp.task(
  "router:copy",
  gulp.series("router:del", function() {
    return prepareRouterData(config.ALL_CONFIGS()).pipe(
      gulp.dest(`${config.dataDir}/build`)
    );
  })
);
...
gulp.task(
  "router:buildGraph",
  gulp.series("router:copy", function() {
    gulp
      .src(["otp-data-container/*", "otp-data-container/.*"])
      .pipe(gulp.dest(`${config.dataDir}/build/croatia`));
