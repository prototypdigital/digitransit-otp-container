  ...
  const p = new Promise((resolve, reject) => {
    if (!fs.existsSync(gtfsFile)) {
      ...
      fs.mkdtemp(`${dataDir}/tmp/router-build-test`, (err, folder) => {
        ...
        r.on('end', () => {
          try {
            const build = exec(`docker run --rm -v ${hostDataDir}/tmp:/opt/opentripplanner/graphs --entrypoint /bin/bash hsldevcom/opentripplanner:${testTag} -c "java -Xmx3G -jar otp-shaded.jar --build graphs/${dir} "`,
