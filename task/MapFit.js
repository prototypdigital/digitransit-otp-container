    ...
    return through.obj(function(file, encoding, callback) {
      ...
      if (config === undefined) {
        process.stdout.write(
          `${gtfsFile} Could not find config for Id:${id}, ignoring fit...\n`
        );
        callback(null, null);
        return;
      }
      const osmFile = `${dataDir}/ready/osm/croatia.pbf`;

      if (!fs.existsSync(osmFile)) {
        process.stdout.write(
          `${osmFile} not available, skipping ${gtfsFile}\n`
        );
        callback(null, null);
        return;
      }
      if (config.fit === false) {
        ...
        run(script, "/data/ready/osm/croatia.pbf", src, dst).then(status => {
          if (status === true) {
            fs.unlinkSync(src);
